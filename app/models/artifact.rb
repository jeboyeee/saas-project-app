class Artifact < ActiveRecord::Base
  before_save :upload_to_s3
  
  attr_accessor :upload #file upload from the view
  belongs_to :project
  
  MAX_FILESIZE = 10.megabytes
  # Add presence validation for name and upload and for name uniqueness to Artifact:
  validates_presence_of :name, :upload
  validates_uniqueness_of :name
  # Add another custom validation for file size:
  validate :uploaded_file_size
  
  private
  #Here is a guide used for doing this (and how the method was constructed): 
  #http://docs.aws.amazon.com/AmazonS3/latest/dev/UploadObjSingleOpRuby.html
  def upload_to_s3
    s3 = Aws::S3::Resource.new
    tenant_name = Tenant.find(Thread.current[:tenant_id]).name
    obj = s3.bucket(ENV['S3_BUCKET']).object("#{tenant_name}/#{upload.original_filename}")
    obj.upload_file(upload.path, acl:'public-read')
    self.key = obj.public_url
  end
  
  # With the uploaded_file_size looking like this under private:
  def uploaded_file_size
   if upload
      errors.add(:upload, "File size must be less than 
      #{self.class::MAX_FILESIZE}") unless upload.size <= self.class::MAX_FILESIZE
   end
  end
  
end
