...
# copied from https://github.com/jekuno/milia/blob/master/app/controllers/registrations_controller.rb
# milia gem in github
...
def create
  ...
  user_params   = sign_up_params_user.merge({ is_admin: true }) #the user signing up creating the org. will be admin, before default is false
  ...
  if !::Milia.use_recaptcha  ||  verify_recaptcha
    ...
    Tenant.transaction  do
      ...
      #Custom Registrations Controller (tut)
       if @tenant.plan == 'premium'
         @payment = Payment.new({ email: user_params["email"],
           ...
           begin
          @payment.process_payment #payment.rb
          @payment.save
          rescue Exception => e #if it doesnt save
